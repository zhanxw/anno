...
AnnotationOutputFile(const std::string& out):headerOutputted(false), totalVariants(0),outputFileName(out) {
    ...
    /* } */
    if (hasSuffix(out, ".gz")) {
      this->fout = new FileWriter(out.c_str(), IO_BGZIP);
    } else {
  ...
  int indexOutput() {
    ...
    if (fileExists(indexFileName) &&
      ...
      fprintf(stderr, "Done: Removed old index file ...\n");
    }
    ...
    switch (aif->getFormat()) {
      ...
      case PLINK:
        ...
        break;
      case EPACTS:
        fprintf(stderr, "EPACTS outputs are not tab-delimited and not indextable for now.\n");
        return -1;
    }
